<?php
	...
	if($_POST['tipo'] == "g"){
		...
		for ($i = 1; $i < $nelem; $i++) {			
	        ...
	        pg_query("insert into kardex_producto values ('$cont_kardex','$arreglo1[$i]','$arreglo2[$i]','$arreglo3[$i]','$arreglo5[$i]','','','','$stock','$precio_compra','$t_precio_compra','$fecha','Ingreso por Factura Compra','Factura Compra','$cont')");
	        pg_query("update producto set stock = '$stock', precio_compra = '$precio_compra', precio_venta = '$arreglo4[$i]' where id_producto = '$arreglo1[$i]'");
	        /**/
		}				
		$data = $cont;
	}else{
